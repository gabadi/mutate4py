Feature: QA — the mutation run loop is observable end-to-end through the CLI
  ...
  #   and any sentinel the fake pytest test writes). No project API — QA never
  ...
  #   hand-written LCOV fixture marking those lines covered; and a FAKE pytest test
  #   (a small test file QA controls, selected via --pytest-args, no shell involved)
  #   whose pass/fail/sleep behavior is scripted so QA can force killed / survived /
  #   timeout / baseline-fail outcomes deterministically.
  #   QA asserts on stdout (header, per-mutant lines, report), exit code, and disk.
  #
  # CONSTRAINTS:
  #   - QA controls the fake pytest test end to end: it can make the run pass
  #     (survived), fail (killed), or sleep past the timeout (timeout), and can
  ...
  #     a user who writes the same test file.
  ...
  #   - timeout outcomes are driven by a fake pytest test that sleeps longer than
  ...
  Scenario Outline: QA drives killed / survived / timeout and sees the status verbatim
    ...
    When the QA agent runs "mutate4py calc.py --lcov cov.info --pytest-args tests --timeout-factor 2"
  ...
  Scenario: QA sees a timed-out mutant counted as Killed, not as a Timeout line
    Given the fake pytest test makes one mutant sleep past the timeout and the rest exit nonzero
    When the QA agent runs "mutate4py calc.py --lcov cov.info --pytest-args tests --timeout-factor 2"
  ...
  Scenario Outline: QA confirms the Survivors block is conditional on survived > 0
    Given the fake pytest test makes "<survivedCount>" of the 2 mutants exit zero and the rest exit nonzero
    When the QA agent runs "mutate4py calc.py --lcov cov.info --pytest-args tests --timeout-factor 2"
  ...
  Scenario: QA sees a failing baseline abort the run with no report and no backup
    Given the fake pytest test exits nonzero on the unmutated baseline run
    When the QA agent runs "mutate4py calc.py --lcov cov.info --pytest-args tests"
  ...
  Scenario: QA confirms the run header lines are present and no workers line appears
    Given the fake pytest test exits nonzero for every mutant
    When the QA agent runs "mutate4py calc.py --lcov cov.info --pytest-args tests --timeout-factor 2"
  ...
  Scenario: QA sees the stale-coverage warning printed before the run header
    ...
    And the fake pytest test exits nonzero for every mutant
    When the QA agent runs "mutate4py calc.py --reuse-coverage --pytest-args tests --timeout-factor 2"
