Feature: QA — parallel --max-workers execution is observable end-to-end through the CLI
  ...
  #   fixture, the .mutate4py/workers/ tree, and any sentinel the fake test command
  ...
  #   hand-written LCOV fixture; and a FAKE test command (a script QA controls) whose
  #   exit code / sleep / file-touch QA scripts to force outcomes and to observe which
  #   working directory each run executes in.
  ...
  #   - QA controls the fake --test-command end to end (exit zero / non-zero / sleep past
  #     timeout), and can branch on whether the source copy currently holds a mutant, so
  ...
  #   - To observe isolation, the fake test command records its own working directory
  #     (e.g. writes `pwd` to a per-run sentinel); QA confirms parallel runs execute under
  #     a .mutate4py/workers/.../worker-<k> path, not the original directory.
  #
  # SEQUENCING:
  ...
  #     fake command that inspects the tree on first invocation) and AFTER the run (via a
  ...
  Scenario Outline: QA confirms the Mutation workers line prints whenever --max-workers is over zero
    Given "runtests.sh" exits nonzero for every mutant while the baseline passes
    When the QA agent runs "mutate4py calc.py --lcov cov.info --max-workers <workers> --test-command ./runtests.sh --timeout-factor 2"
  ...
  Scenario: QA sees --max-workers clamped to the selected-site count on the workers line
    Given "runtests.sh" exits nonzero for every mutant while the baseline passes
    When the QA agent runs "mutate4py calc.py --lcov cov.info --max-workers 9 --test-command ./runtests.sh --timeout-factor 2"
  ...
  Scenario Outline: QA drives killed / survived / timeout under --max-workers and sees the status
    ...
    When the QA agent runs "mutate4py calc.py --lcov cov.info --max-workers 4 --test-command ./runtests.sh --timeout-factor 2"
  ...
  Scenario: QA sees the same report tallies and survivor set across repeated parallel runs
    Given "runtests.sh" makes "1" of the 4 mutants exit zero and the rest exit nonzero
    When the QA agent runs "mutate4py calc.py --lcov cov.info --max-workers 4 --test-command ./runtests.sh --timeout-factor 2" twice
  ...
  Scenario: QA confirms the test command runs under a per-worker copy, not the original dir
    Given "runtests.sh" records its working directory to a sentinel and exits nonzero
    When the QA agent runs "mutate4py calc.py --lcov cov.info --max-workers 4 --test-command ./runtests.sh --timeout-factor 2"
  ...
  Scenario: QA sees the worker tree present during the run and removed afterward
    Given "runtests.sh" checks for a ".mutate4py/workers/" tree on its first call and exits nonzero
    When the QA agent runs "mutate4py calc.py --lcov cov.info --max-workers 4 --test-command ./runtests.sh --timeout-factor 2"
    Then "runtests.sh" observed a ".mutate4py/workers/" tree during the run
