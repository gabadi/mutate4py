...
2026-06-28 | c8748468 | hardender | tool-error | promoted→ledger-swarmforge | pending | RTK swallows stdout when underlying process calls process.exit(1) before writing; use bare command without rtk when zero output is unexpected
2026-06-28 | c8748468 | hardender | convention-gap | promoted→ledger-swarmforge | pending | Before writing handoff to: field, verify role name case with ls swarmforge/roles/ — names are case-sensitive (QA not qa)
2026-06-28 | c8748468 | hardender | convention-gap | promoted→ledger-swarmforge | pending | Do not bun add -d <local-path> tools to package.json for analysis — install ephemerally, run tool, then revert package.json and bun.lock with git checkout --
2026-06-28 | 5707a6e5 | coder | convention-gap | promoted→ledger-swarmforge | pending | After merge-result handoff, diff public function signatures of touched modules against pre-merge state before running tests
2026-06-29 | eaa9acfe | integrator | convention-gap | promoted→ledger-swarmforge | pending | Integrator: explicit git push -u origin feat/<initiative> step must precede gh pr create; push is not implicit
2026-06-29 | eaa9acfe | integrator | convention-gap | promoted→ledger-swarmforge | pending | Before pushing to CI, run both ruff check --fix AND ruff format in a single pass (they are separate CI steps)
